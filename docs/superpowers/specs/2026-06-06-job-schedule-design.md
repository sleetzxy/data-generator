...
> **2026-06-07 更新：** 调度配置已合并到新建/编辑弹窗，与 YAML 一次保存；列表仅保留 **Cron** 列（无独立「调度」列）；已移除「配置路径」列；任务列表与运行日志支持分页。自定义任务的显示名称由「任务名称」输入框维护，不写进 YAML 编辑区；新建时配置文件名默认使用自动生成的 ASCII `id`。
...
| Cron | 启用时显示表达式，否则 `—` |
| 最近状态 | 最近一次运行状态 |
| 操作 | 运行、更多（查看/预览/日志/编辑/删除） |

列表排序：内置优先（fileName 字母序），自定义按 `createdAt` 倒序。超过 20 条分页。

### 7.2 新建/编辑弹窗（自定义 Job）

- **任务名称**：写入 YAML `name`，编辑区不展示该字段
- **YAML 内容**：业务配置；不含顶层 `id`/`name`（新建时 `id` 自动生成）
- **定时调度**：启用开关 + Cron + 下次执行时间；与 YAML 同请求保存（`schedule` 字段）
- 内置 Job：只读查看，调度来自 YAML，不可在 UI 修改

独立 `PUT .../schedule` 端点仍保留，供 API 使用。

### 7.3 手动运行

「运行」按钮：已启用 Cron 时 confirm 提示立即执行一次；若同配置正在运行则排队并 toast。

### 7.4 运行日志

「日志」弹窗：运行记录分页（10 条/页）；展开后详细日志分页（50 条/页）。
