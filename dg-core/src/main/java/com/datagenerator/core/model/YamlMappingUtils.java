...
final class YamlMappingUtils {
    ...
    static TableSchema toSchemaDefinition(Map<String, Object> source) {
    ...
    static SeedLinkDefinition toSeedLinkDefinition(Map<String, Object> source) {
        ...
        link.setMatch(asString(source.get("match")));
        Object sourcesValue = source.get("sources");
        if (sourcesValue instanceof List<?> sourceList) {
            List<SeedLinkSourceDefinition> sources = new ArrayList<>();
            for (Object item : sourceList) {
                sources.add(toSeedLinkSourceDefinition(asMap(item)));
            }
    ...
    static SeedLinkSourceDefinition toSeedLinkSourceDefinition(Map<String, Object> source) {
        SeedLinkSourceDefinition definition = new SeedLinkSourceDefinition();
        definition.setMatch(asString(source.get("match")));
        definition.setColumn(asString(source.get("column")));
        definition.setQuery(asString(source.get("query")));
        definition.setTable(asString(source.get("table")));
        definition.setProfile(asString(source.get("profile")));
        return definition;
    }
