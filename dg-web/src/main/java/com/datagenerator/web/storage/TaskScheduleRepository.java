...
public class TaskScheduleRepository {
    ...
    public record TaskScheduleRecord(String configPath, boolean enabled, String cron, String updatedAt) {
    }
    ...
    public void ensureCreatedAt(String configPath, String createdAt) {
        jdbcTemplate.update("""
                ...
                VALUES (?, 0, NULL, ?, ?)
                ON CONFLICT(config_path) DO UPDATE SET
                    created_at = COALESCE(task_schedules.created_at, excluded.created_at)
                """,
                configPath,
                createdAt,
                createdAt);
    ...
    public Optional<String> findCreatedAt(String configPath) {
        List<String> results = jdbcTemplate.query(
                "SELECT created_at FROM task_schedules WHERE config_path = ?",
    ...
    public Optional<TaskScheduleRecord> findByConfigPath(String configPath) {
        List<TaskScheduleRecord> results = jdbcTemplate.query("""
                SELECT config_path, enabled, cron, updated_at
                FROM task_schedules WHERE config_path = ?
                """, this::mapRow, configPath);
        return results.isEmpty() ? Optional.empty() : Optional.of(results.get(0));
    }

    public List<TaskScheduleRecord> findAll() {
        return jdbcTemplate.query("""
                SELECT config_path, enabled, cron, updated_at
                FROM task_schedules ORDER BY config_path
                """, this::mapRow);
    }

    public void deleteByConfigPath(String configPath) {
        jdbcTemplate.update("DELETE FROM task_schedules WHERE config_path = ?", configPath);
    }

    public void deleteOrphans(Set<String> validConfigPaths) {
        if (validConfigPaths.isEmpty()) {
            jdbcTemplate.update("DELETE FROM task_schedules");
            return;
        }
        String placeholders = String.join(",", validConfigPaths.stream().map(path -> "?").toList());
        jdbcTemplate.update(
                "DELETE FROM task_schedules WHERE config_path NOT IN (" + placeholders + ")",
                validConfigPaths.toArray());
    }

    private TaskScheduleRecord mapRow(java.sql.ResultSet rs, int rowNum) throws java.sql.SQLException {
        return new TaskScheduleRecord(
