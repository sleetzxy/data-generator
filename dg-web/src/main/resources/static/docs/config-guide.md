...
4. 运行中任务会**按批次**更新进度（`written_rows`）与日志；自动刷新时保留滚动位置
...
| `job.batch-size` | 1000 | 写入批次（每批 flush 后更新进度与日志） |
| `job.thread-pool-size` | 4 | 异步线程数 |
...
A：确认顶层 `seeds[].name` 与字段 `generator.source` 一致；`strategy: seed` 须显式指定 `source`。
