...
> **交付：** 通过回复末尾的 YAML 代码块维护会话草稿（见 **output-format**）。用户要求新建时调用 `saveDraftJobDefinition`，编辑时调用 `updateDraftJobDefinition`，删除时调用 `deleteJobDefinition`。
>
> **确认：** 缺什么问什么；用户已给出或可通过 Tool 获取的信息不要重复确认。
