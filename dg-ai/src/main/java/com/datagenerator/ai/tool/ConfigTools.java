...
public class ConfigTools {
    ...
    public String startConfigDraft(
        ...
        if (draftId == null || draftId.isBlank()) {
            return "错误：draftId 不能为空";
        }
        try {
            draftManager.createDraft(rc, draftId, headerYaml.trim());
            return "草稿「" + draftId + "」已创建。接下来请用 addTableToDraft 添加表定义。";
        } catch (Exception e) {
    ...
    public String saveConfigDraft(
        ...
        try {
            ...
            if (!emptyCols.isEmpty()) {
                return "以下 table 尚未定义任何字段，无法保存：" + emptyCols
            ...
            if (!vr.valid()) {
                return "校验失败，未保存。草稿保留在 workspace 中可继续修改：\n"
                        + String.join("\n", vr.errors());
            }
            ...
            if (saved == null) {
                return "保存失败：服务端返回空结果，请检查 dg-web 连接状态";
            }
    ...
    public String discardConfigDraft(
        ...
        try {
            ...
            return "草稿「" + draftId + "」已废弃。";
        } catch (Exception e) {
            log.error("废弃草稿失败: draftId={}", draftId, e);
            return "废弃草稿失败：" + e.getMessage();
        }
    }
    ...
    public String addTableToDraft(
        ...
        try {
            ...
            return "table「" + tableName + "」" + extra + " 已添加到草稿「" + draftId + "」。";
        } catch (Exception e) {
            log.error("添加 table 失败: draftId={}", draftId, e);
            return "添加 table 失败：" + e.getMessage();
        }
    }
    ...
    public String removeTableFromDraft(
        ...
        try {
            ...
            return "table「" + tableName + "」已从草稿「" + draftId + "」中删除。";
        } catch (Exception e) {
            log.error("删除 table 失败: draftId={}, tableName={}", draftId, tableName, e);
            return "删除 table 失败：" + e.getMessage();
        }
    }
    ...
    public String addTableMetaToDraft(
        ...
        try {
            ...
            return "table 壳「" + tableName + "」已创建（0 字段）。请用 addFieldsToTable(draftId=\""
                    + draftId + "\", tableName=\"" + tableName + "\", fieldsYaml=...) 逐批追加字段。";
        } catch (Exception e) {
            log.error("添加 table meta 失败: draftId={}", draftId, e);
            return "添加 table meta 失败：" + e.getMessage();
        }
    }
    ...
    public String setDraftConstraints(
        ...
        try {
            ...
            return "constraints 已设置。";
        } catch (Exception e) {
            log.error("设置 constraints 失败: draftId={}", draftId, e);
            return "设置 constraints 失败：" + e.getMessage();
        }
    }
    ...
    public String setDraftSeeds(
        ...
        try {
            ...
            return "设置 seeds 失败：" + e.getMessage();
        }
    }
